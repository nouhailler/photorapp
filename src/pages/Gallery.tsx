...
const Gallery: React.FC = () => {
  ...
  const groupedPhotos = photos.reduce((acc, photo) => {
    const date = photo.date; // Use simplified date for demo
      ...
      {Object.entries(groupedPhotos).map(([date, datePhotos]) => (
        <section key={date}>
          <div className="px-container-padding mb-3 flex justify-between items-end">
            <h2 className="text-headline-md font-bold text-on-surface">{date === '2026-06-08' ? 'Today' : date}</h2>
            <span className="text-label-md font-bold text-primary cursor-pointer hover:underline">Select</span>
          </div>
          <PhotoGrid photos={datePhotos} onPhotoClick={handlePhotoClick} />
        </section>
      ))}
